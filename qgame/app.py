...
@app.route('/admin/import_gseb_questions', methods=['POST'])
@login_required
def admin_import_gseb_questions():
    if current_user.role != 'admin':
        return redirect(url_for('user_dashboard'))
    ...
    try:
        ...
        for item in data:
            ...
            # Check duplicate based on exact English question and standard/subject
            ...
            # Create category just for legacy view (if we want them visible in old views)
        ...
        db.session.commit()
        flash(f'Successfully imported {count} questions! (Skipped {duplicate} duplicates)', 'success')
    except Exception as e:
        db.session.rollback()
        flash(f'Error importing JSON: {e}', 'error')
